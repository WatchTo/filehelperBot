...
WX_FILEUPLOAD_HOST = "https://file.wx.qq.com"
...
class Message:
    ...
    def sync_msg_check(self):
        ...
        params = {
            ...
            'uin': self.uin,
            'deviceid': Utils.generate_device_id(),
            'synckey': "|".join([f"{item['Key']}_{item['Val']}" for item in self.sync_key['List']]),
    ...
    def receive_msg(self):
        ...
        if resp:
            ...
            if data['BaseResponse']['Ret'] == 0:
                self.sync_key = data['SyncCheckKey']
                if data['AddMsgList']:
                    ...
                    for msg in data['AddMsgList']:
                        if msg['MsgType'] == 51:
                            continue
                        print(msg)
                        if msg['MsgType'] == 1 and msg['Content'] == '测试':
                            self.send_msg(file_path=r"test.png")
                    
                    return data['AddMsgList']
            else:
                raise ValueError("Webwxsync failed")

    def wait_msg(self, filehelper):
        """监听消息"""
        while True:
            has_msg = self.sync_msg_check()
            if has_msg == 1:
                message_list = self.receive_msg()
                if message_list:
                    for message in message_list:
                        handlers = filehelper.get_handlers(message['MsgType'])
                        for handler in handlers:
                            handler(message)
                            break # 只要有handler响应就跳过其他的handler
            elif has_msg == -1:
                print("Login expired")
                break
...
class WXRequest:
    ...
    def fetch(self, url, method="get", params=None, data=None, json=None, timeout=50):
...
class WXFilehelper(BaseBot):
    def __init__(self):
        super().__init__()
        self.wx_req = WXRequest()
        self.message = Message()
        self.status = self.wait_login()
    ...
    def run(self):
        self.message.wait_msg(self)

# def save(filehelper: WXFilehelper):
#     if filehelper.status != -1:
#         with open('save.pkl', 'wb') as f:
#             pickle.dump(filehelper, f)
...
if __name__ == '__main__':
    ...
    @filehelper.filter(re.compile(".*?hello.*?"))
    def test(msg):
        filehelper.message.send_msg("你也好呀")

    @filehelper.image
    def test(msg):
        filehelper.message.send_msg("接收了图片")
    
    @filehelper.bqb
    def test(msg):
        filehelper.message.send_msg("接收了表情包")

    @filehelper.voice
    def test(msg):
        filehelper.message.send_msg("接收了语音")
    
    @filehelper.video
    def test(msg):
        filehelper.message.send_msg("接收了视频")

    @filehelper.card
    def test(msg):
        filehelper.message.send_msg("接收了卡片消息")
